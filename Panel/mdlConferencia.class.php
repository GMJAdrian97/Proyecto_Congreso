<?php
    ...
    class Conferencia extends Sistema{
        ...
        public function readEvento($id_evento){
            $this->connect();
            $sql = "SELECT cp.id_evento, cp.id_conferencia_p, c.id_conferencia, c.titulo, cp.fecha, cp.hora_inicio, cp.hora_fin, 
                    COUNT(DISTINCT ip.id_participante) as inscritos from conferencia_programacion cp 
                    JOIN conferencia c on cp.id_conferencia=c.id_conferencia 
                    left join inscripcion_participante ip on cp.id_conferencia_p = ip.id_conferencia_p WHERE cp.id_evento=:id_evento;";
            $stmt = $this->con->prepare($sql);
